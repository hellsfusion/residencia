...
$name = 'Documentos Generados Plantillas';
$page = 'documentos_generados_plantillas';
$table = 'documentos_generados_plantillas';
$rowConfig = null;
// var_dump($_GET);
if ($_GET['FAID'] != null) {
        ...
        <div class="row">
         ...
         <div class="col-md-9">
          <div class="form-group">
           ...
           <input type="text" class="form-control" id="titulo" name="datos[titulo]" required value="<?= ($rowConfig != null ? $rowConfig['titulo'] : null) ?>">
          </div>
         </div>
         <div class="col-md-12">
          <div class="form-group">
           <label for="nombre">Documento</label>
           <textarea name="datos[contenido]" class="summernote"><?= ($rowConfig != null ? $rowConfig['contenido'] : null) ?></textarea>
          </div>
         </div>
        </div>
       </div>
       ...
       <table class="table table-striped">
        ...
        <tbody>
         <?php foreach ($rowData as $row) { ?>
          <tr>
           <td><?= $row['id'] ?></td>
           <td><?= funcionMaster($row['idEdificio'], 'id', 'nombre', 'edificios') ?> </td>
           <td><?= $row['titulo'] ?></td>
           <td class="text-center" style="width: 100px">
            <button class="btn btn-secondary btn-sm btn-block" onclick="window.open('<?= $page ?>?FAID=<?= base64_encode($row['id']) ?>', '_self');" title="Editar">
             <i class="fas fa-edit"></i>
             Editar
            </button>
            <button class="btn btn-primary btn-sm btn-block" onclick="window.open('documentos_generar?FAID=<?= base64_encode($row['id']) ?>', '_self');" title="Crear Documento">
